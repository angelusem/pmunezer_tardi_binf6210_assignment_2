...
# we could use janitor in this instance 
install.packages("here"); install.packages("janitor")
df_tardi <- readr::read_tsv(here::here("data", "Tardigrada_BOLD_data.tsv"))
df_tardi <- janitor::clean_names(df_tardi)  # creates country_ocean automatically
...
df_tardi.countries_shannon <- df_tardi.countries %>% 
  ...
  group_by(country_ocean) %>% 
  summarize(shannon = diversity(count, index = "shannon"),simpson = diversity(count, index = "simpson")) %>% 
